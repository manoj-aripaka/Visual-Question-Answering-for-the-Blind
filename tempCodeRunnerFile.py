...
class NetFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        for layer in self.layers:
            getattr(self.model, layer).register_forward_hook(self.save_features(layer))
...
def visualize_features(image_path, net, device):
    ...
    # Edge map (you may need to implement edge detection)
    # For simplicity, let's use a basic gradient magnitude
    img_gray = np.array(image.convert('L'))
    gradient = np.gradient(img_gray)
    edge_map = np.sqrt(np.square(gradient[0]) + np.square(gradient[1]))
    ...
    for i, layer in enumerate(net.layers):
        feature = features[layer][0].sum(dim=0).cpu().numpy()
        feature = (feature - feature.min()) / (feature.max() - feature.min())
        axs[(i+2)//3, (i+2)%3].imshow(feature, cmap='gray')
        axs[(i+2)//3, (i+2)%3].set_title(f'Layer {i+1}')
...
image_path = 'path_to_your_moth_image.jpg'
visualize_features(image_path, net, device)
...
def plot_feature_maps(feature_maps, num_maps=64):
    for fmap in feature_maps:
        fmap = fmap[0]  # Get the feature maps from the first image in the batch
        fmap = fmap.detach().cpu().numpy()  # Convert to numpy array

        # Plot a grid of feature maps
        square = int(np.ceil(np.sqrt(num_maps)))
...
def feature_ext():
    ...
    dummy_input = torch.ones(1, 3, config['img_size'], config['img_size']).to(device)
    with torch.no_grad():
        dummy_output = net(dummy_input)

    att_features_shape = (
        len(data_loader.dataset),
        dummy_output[-1].size(1),
        dummy_output[-1].size(2),
        dummy_output[-1].size(3)
    )

    noatt_features_shape = (
        len(data_loader.dataset),
        dummy_output[-1].view(1, -1).size(1)
    )
    ...
    with torch.no_grad():
        for i, inputs in enumerate(tqdm(data_loader)):
            inputs_img = inputs['visual'].to(device, non_blocking=True)
            feature_maps = net(inputs_img)

            no_att_feat = feature_maps[-1].view(inputs_img.size(0), -1)

            print(f"Shape of no_att_feat: {no_att_feat.shape}")
            print(f"Expected shape in h5_noatt: {h5_noatt[idx:idx + inputs_img.size(0)].shape}")

            h5_noatt[idx:idx + inputs_img.size(0)] = no_att_feat.data.cpu().numpy().astype('float16')
            h5_att[idx:idx + inputs_img.size(0), :, :, :] = feature_maps[-1].data.cpu().numpy().astype('float16')
            img_names[idx:idx + inputs_img.size(0)] = inputs['name']

            idx += inputs_img.size(0)

            # Plot feature maps for the first batch
            if i == 0:
